...
namespace Z3.LinqBinding
{
    ...
    public class Theorem
    {
        ...
        private static object GetSolution(Type t, Context context, Model model, Environment environment)
        {
            ...
            if (t.GetCustomAttributes(typeof(CompilerGeneratedAttribute), false).Any())
            {
                ...
                foreach (var parameter in environment.Properties.Keys)
                {
                    ...
                    else if (parameter.PropertyType == typeof(long))
                        field.SetValue(result, ((IntNum)val).Int64);
                    else
                        throw new NotSupportedException("Unsupported parameter type for " + parameter.Name + ".");
                }
        ...
        {            //
            ...
            switch (expression.NodeType)
            {
                case ExpressionType.Conditional:
                    return VisitITE(context, environment, (ConditionalExpression)expression, param, (ctx, i, t, e) => ctx.MkITE(i,t,e));
                //case ExpressionType.Coalesce:
                case ExpressionType.Coalesce:
                    return VisitBinary(context, environment, (BinaryExpression)expression, param, (ctx, a, b) => ctx.MkImplies((BoolExpr)a, (BoolExpr)b));
        ...
        private Expr VisitITE(Context context, Environment environment, ConditionalExpression expression, ParameterExpression param, Func<Context, BoolExpr, Expr, Expr, Expr> ctor)
        {
            return ctor(context, 
                (BoolExpr)Visit(context, environment, expression.Test, param),
                Visit(context, environment, expression.IfTrue, param),
                Visit(context, environment, expression.IfFalse, param)
            );
        }
